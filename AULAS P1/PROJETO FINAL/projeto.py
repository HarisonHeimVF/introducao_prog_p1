...
def translate_advice(advice, target_lang):
    try:
        translator = GoogleTranslator(source='pt', target='portuguese')
...
def show_multiple_advices():
    ...
    for _ in range(num_advices):
        ...
        if advice:
            # Exibir o conselho diretamente em português (sem tradução)
            all_advices.append(f"Conselho {advice_id} (Português): {advice}")
...
def translate_saved_advice():
    selected_lang = language_var.get()  # Pega o idioma selecionado no menu
    if selected_lang == "Inglês":
        target_lang = 'en'
    ...
    if advice_text:
        # Pega o texto em português e remove "(Português)" para traduzir
        advice_text_pt = advice_text.split("(Português): ")
        translated_advice = translate_advice(advice_text_pt, target_lang)
        if translated_advice:
            result_text.set(f"{advice_text}\n\nTraduzido para {selected_lang}: {translated_advice}")
    else:
        messagebox.showwarning("Aviso", "Nenhum conselho disponível para traduzir.")
...
def save_advice():
    ...
    if advice_text:
        ...
        messagebox.showwarning("Aviso", "Nenhum conselho disponível para salvar.")
...
def create_gui():
    ...
    global language_var
    language_var = tk.StringVar(value="Inglês")
